		...
		public void Draw1(Chart chart1, Chart chart2)
		{
			...
			for (int j = 0; j < tValues.Length; ++j)
			{
				...
				for (int i = 0; i < ParamsHelper.I_x + 1; ++i)
				{
                    ...
                    chart1.Series[j].Points.AddXY(Solver.Xarr[i], Solver.V[(int)(tValues[j]  / Solver.h_t), i]);
                }
            }

			//фиксируем точку
			double [] xValues = { 0, 1, 3, 4, 5};
			for (int j = 0; j < xValues.Length; ++j)
			{
				...
				{
					var f = (xValues[j]  / Solver.h_x);

                    double p = (int)Math.Round(Solver.Xarr[(int)(xValues[j]  / Solver.h_x)]);
